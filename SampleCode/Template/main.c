...
void CONFIG0_Init(void)
{
    uint32_t au32Config[2];

	uint16_t ICELOCK = BIT11;
	uint16_t LOCK = BIT1;
	...
	#if defined (ICE_LOCK_BIT)
    if (au32Config[0] & ICELOCK)
    {
        printf("%s : ICELOCK\r\n",__FUNCTION__);
		while(!UART_IS_TX_EMPTY(UART0));
		
        FMC_ENABLE_CFG_UPDATE();       		/* Enable User Configuration update. */
        au32Config[0] &= ~ICELOCK; 
	...
	#if defined (SECURITY_LOCK_CONTROL)
    if (au32Config[0] & LOCK)
    {
        printf("%s : LOCK\r\n",__FUNCTION__);		
		while(!UART_IS_TX_EMPTY(UART0));
